			...
			iter1 != p_MetaDataSet->getwFuncList()->end(); iter1++)
	{
		...
		for(auto iter2 = CurrentFunc->getwBBList()->begin(); 
			...
			for(auto iter3 = CurrentBB->getwInstList()->begin(); 
				...
				{	
					if(CurrentFunc->getName() == std::string(target_symbol))
					{
						std::cout << "MAIN_RETURN: " <<  CurrentFunc->getName() << std::endl;
						CurrentBB->setType(wBasicBlock::MAIN_RETURN);
				...
				iter1++)
		{
					...
					iter2++)
			{
						...
						currentWBB == *(Wf->getwBBList()->begin()))
				{
					...
					SignatureData* new_sd = new SignatureData;
					new_sd->basicblock = currentBB;
					new_sd->node_id = currentWBB->getID();
					new_sd->type = (unsigned int)(currentWBB->getType());
					
					p_CompactImplantedBBList->push_back(new_sd);
...
bool Analyzer::FineGrainedAnalysisForMonitoringCodeGenerator2()
{
			...
			iter1++)
	{
				...
				iter2++)
		{
			...
			/* RETURN BasicBlcok */	
			if(currentBB->getType() == wBasicBlock::BB_type::MAIN_RETURN)
			{
			...
			else if(currentBB->getType() == wBasicBlock::BB_type::RETURN)
			{
			...
			else
			{
				...
				std::cout << " [ERROR] function: " << currentF->getName() << ", basicblock: " << currentBB->getID() << std::endl;
				return false;
			}
...
bool Analyzer::FineGrainedAnalysisForMonitoringCodeGenerator()
{
			...
			iter1++)
	{
				...
				iter2++)
		{
			...
			else if(currentBB->getType() == wBasicBlock::BB_type::MAIN_RETURN)
			{
				newCompactBBInfo
					= new CompactBBInfo(
							currentBB->getID(),
...
bool Analyzer::Debug_Show_Signal_CompactBBInfo()
{
			...
			iter++)
	{
		...
		std::cout << "node_id: " << std::hex << sd->node_id << std::endl;
		std::cout << "node_type: " << sd->type << std::endl;
	}

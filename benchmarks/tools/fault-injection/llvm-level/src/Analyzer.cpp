...
bool Analyzer::fault_injection()
{
	
	llvm::BasicBlock* from = nullptr;
	llvm::BasicBlock* to = nullptr;

	std::cout << "select_two_basicblock" << std::endl;
	select_two_basicblock(&from, &to);
	
	std::cout << "branch_injection_from_to" << std::endl;
	branch_injection_from_to(from, to);
...
bool Analyzer::branch_injection_from_to(llvm::BasicBlock* from, llvm::BasicBlock* to)
{

	//IRBuilder<> branch_builder(from);
	//llvm::Value* br_inst_value = branch_builder.CreateBr(to);
	llvm::BranchInst* br_inst = llvm::BranchInst::Create(to); 
	br_inst->setParent(from);
	//	new llvm::BranchInst(to);
	//llvm::Instruction* br_inst = cast<Instruction>(br_inst_value);
	
	from->getInstList().push_front(br_inst);


	return true;
}

bool Analyzer::select_two_basicblock(llvm::BasicBlock** from, llvm::BasicBlock** to)
{

	unsigned int numberofbasicblocks = numberofbasicblock();
	
	//llvm::BasicBlock** BasicBlockArray;
	std::vector<llvm::BasicBlock*> BasicBlockArr;


	/* 1 */
	getBasicBlockArray(BasicBlockArr, numberofbasicblocks);
	

	unsigned int from_number = 0;
	unsigned int to_number = 0;

	/* 2 */
	from_number = getRandomNumber(numberofbasicblocks);
	to_number = getRandomNumber(numberofbasicblocks);
	
	std::cout << " BasicBlock#" << from_number << " to BasicBlock# " << to_number << std::endl;
	...
	/* 3 */
	std::cout << "get BasicBlocks is starting.." << std::endl;
	(*from) = BasicBlockArr[from_number];

	to_number = from_number+1;

	(*to) = BasicBlockArr[to_number];
	std::cout << "get BasicBlocks has finished!" << std::endl;

	/*
	(*from) = *(BasicBlockArray +from_number);
	(*to) = *(BasicBlockArray + to_number);
	*/

	llvm::Function* from_function = (*from)->getParent();
	llvm::Function* to_function = (*to)->getParent();

	std::cout << " from_function: " << from_function->getName().str() << std::endl;
	std::cout << " to_function: " << to_function->getName().str() << std::endl;

	//freeBasicBlockArray(BasicBlockArray);
...
bool Analyzer::freeBasicBlockArray(llvm::BasicBlock** arr)
{
	free(arr);
}


unsigned int Analyzer::getRandomNumber(unsigned int numberofbasicblocks)
{
	//std::cout << "getRandomNumber is starting..." << std::endl;
	srand((unsigned int)time(NULL));

	static unsigned number = 7781279;

	int adjustment_value_front_margine = 5;
	int adjustment_value_back_margine = 5;
	//std::cout << "getRandomNumber has finished!" << std::endl;

	number *= number;

	unsigned int return_value = (rand() * number)% (numberofbasicblocks - adjustment_value_front_margine - adjustment_value_back_margine ) + adjustment_value_front_margine;
	return return_value;
}

//bool Analyzer::getBasicBlockArray(llvm::BasicBlock*** BasicBlockArray, unsigned int numberofbasicblocks)
bool Analyzer::getBasicBlockArray(std::vector<llvm::BasicBlock*>& BasicBlockArray, unsigned int numberofbasicblocks)
{
	//(*BasicBlockArray) = (llvm::BasicBlock**)malloc(sizeof(llvm::BasicBlock*) * numberofbasicblocks);
	
	std::cout << "getBasicBlockArray is starting..!" << std::endl;
			...
			iter1 != m->getFunctionList().end(); iter1++) 
	{
		...
		{
			llvm::BasicBlock& currBB = (*iter2);
			//(*BasicBlockArray)[iter] = (&currBB);
			BasicBlockArray.push_back(&currBB);
			iter++;
		}
	}

	std::cout << "getBasicBlockArray has finished!" << std::endl;
	return true;
}


unsigned int Analyzer::numberofbasicblock()
{ 
